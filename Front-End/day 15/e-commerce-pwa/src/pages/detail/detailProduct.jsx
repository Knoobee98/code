...
export default function DetailProduct(){
    ...
    let onGetData = async() => {
        try{
            let response = await axios.get(`https://my-json-server.typicode.com/Knoobee98/jsonServer-Ecommerce-apps/products/${productId.id}`)
    ...
    let onSubmit = async() => {
        try{
            ...
            let checkCart = await axios.get(`https://my-json-server.typicode.com/Knoobee98/jsonServer-Ecommerce-apps/cart?product_id=${data.id}`)
            if(checkToken){
                if(checkCart.data.length === 0){
                    let addToCart = await axios.post('https://my-json-server.typicode.com/Knoobee98/jsonServer-Ecommerce-apps/cart', dataTosend)
                    console.log(addToCart);
                } else {
                    let newQuantity = checkCart.data[0].qty + 1
                    let updateQty = await axios.patch(`https://my-json-server.typicode.com/Knoobee98/jsonServer-Ecommerce-apps/cart/${checkCart.data[0].id}`, {qty: newQuantity})
