...
let App = () => {
    ...
    let checkIsLogin = async() => {
      try{
        ...
        if(getTokenid){
          let response = await axios.get(`https://my-json-server.typicode.com/Knoobee98/jsonServer-Ecommerce-apps/users?id=${getTokenid}`)
    ...
    let onLogin = async(inputUsername,inputPassword) => {
      ...
      try {
        let response = await axios.get(
          `https://my-json-server.typicode.com/Knoobee98/jsonServer-Ecommerce-apps/users?username=${inputUsername}&password=${inputPassword}`
        );
